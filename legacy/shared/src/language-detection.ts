import { UniversalTraceData, TraceDataAdapter, LanguageMetadata } from './universal-trace';
...
export class LanguageDetector {
  detectFromTrace(trace: any): LanguageDetectionResult {
    ...
    let language: UniversalTraceData['language'] = 'javascript'; // default
    let confidence = 0;
  ...
  private hasSnakeCaseKeys(obj: any): boolean {
    if (typeof obj !== 'object' || obj === null) return false;
    
    const keys = Object.keys(obj);
    const snakeCaseKeys = keys.filter(key => 
      key.includes('_') && !key.includes('-') && key === key.toLowerCase()
    );
    
    return snakeCaseKeys.length > keys.length * 0.3; // 30% threshold
  }

  private hasCamelCaseKeys(obj: any): boolean {
    if (typeof obj !== 'object' || obj === null) return false;
    
    const keys = Object.keys(obj);
    const camelCaseKeys = keys.filter(key => 
      /^[a-z][a-zA-Z0-9]*$/.test(key) && /[A-Z]/.test(key)
    );
...
export class LegacyTraceAdapter implements TraceDataAdapter {
  getVersion(): string {
  ...
  private mapOperation(legacyTrace: TraceData): string {
    if (legacyTrace.type) {
      return legacyTrace.type;
    }
    if (legacyTrace.metadata?.operation) {
      return legacyTrace.metadata.operation as string;
    }
    return 'unknown_operation';
  }

  private mapFramework(legacyTrace: TraceData): UniversalTraceData['framework'] {
    if (legacyTrace.metadata?.framework) {
      const framework = legacyTrace.metadata.framework as string;
      if (['langchain', 'llamaindex', 'autogen', 'crewai', 'flowise'].includes(framework)) {
        return framework as UniversalTraceData['framework'];
      }
    }
    return 'custom';
  }

  private mapStatus(status: TraceData['status']): UniversalTraceData['status'] {
    switch (status) {
  ...
  private mapLegacyStatus(status: UniversalTraceData['status']): TraceData['status'] {
    switch (status) {
  ...
  private mapTraceEventType(universalTrace: UniversalTraceData): TraceData['type'] {
    const operation = universalTrace.operation.toLowerCase();
    
    if (operation.includes('chain')) return 'chain_start';
    if (operation.includes('prompt')) return 'prompt';
    if (operation.includes('response')) return 'response';
    if (operation.includes('function') || operation.includes('tool')) return 'function_call';
    if (operation.includes('agent')) return 'agent_step';
    if (universalTrace.status === 'error') return 'error';
    
    return 'chain_start'; // default
  }

  private extractLanguageMetadata(legacyTrace: TraceData): LanguageMetadata | undefined {
    if (legacyTrace.metadata?.language_metadata) {
      return legacyTrace.metadata.language_metadata as LanguageMetadata;
    }
    return undefined;
  }

  private extractContext(legacyTrace: TraceData): UniversalTraceData['context'] | undefined {
    ...
    return {
      ...
      custom_attributes: metadata.custom_attributes as Record<string, any>
    };
  }
}
...
export const legacyTraceAdapter = new LegacyTraceAdapter();
