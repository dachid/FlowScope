...
const ExportModal = ({ isOpen, onClose }) => {
    const { traces, filteredTraces, currentSession } = (0, debugger_1.useDebuggerStore)();
    ...
    return (<div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
      <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[80vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
            { format: 'json', icon: lucide_react_1.Code, label: 'JSON', desc: 'Structured data format' },
            { format: 'csv', icon: lucide_react_1.Database, label: 'CSV', desc: 'Spreadsheet compatible' },
            { format: 'markdown', icon: lucide_react_1.FileText, label: 'Markdown', desc: 'Documentation format' },
            { format: 'html', icon: lucide_react_1.FileText, label: 'HTML', desc: 'Web page format' }
        ].map(({ format, icon: Icon, label, desc }) => (<button key={format} onClick={() => setExportOptions(prev => ({ ...prev, format: format }))} className={`p-3 border rounded-lg text-left transition-colors ${exportOptions.format === format
          ...
          <div>
            ...
            <div className="space-y-2">
              <label className="flex items-center gap-2">
                <input type="checkbox" checked={exportOptions.includeMetadata} onChange={(e) => setExportOptions(prev => ({ ...prev, includeMetadata: e.target.checked }))} className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"/>
                <span className="text-sm">Include metadata</span>
              </label>
              <label className="flex items-center gap-2">
                <input type="checkbox" checked={exportOptions.includeTimestamps} onChange={(e) => setExportOptions(prev => ({ ...prev, includeTimestamps: e.target.checked }))} className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"/>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-3">
              <lucide_react_1.Filter className="w-4 h-4 inline mr-1"/>
              Additional Filters
            </label>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs text-gray-500 mb-1">Trace Type</label>
                <select value={exportOptions.filterByType} onChange={(e) => setExportOptions(prev => ({ ...prev, filterByType: e.target.value }))} className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
              ...
              <div>
                ...
                <select value={exportOptions.filterByStatus} onChange={(e) => setExportOptions(prev => ({ ...prev, filterByStatus: e.target.value }))} className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="datetime-local" value={exportOptions.dateRange.start} onChange={(e) => setExportOptions(prev => ({
            ...prev,
            dateRange: { ...prev.dateRange, start: e.target.value }
              ...
              <div>
                ...
                <input type="datetime-local" value={exportOptions.dateRange.end} onChange={(e) => setExportOptions(prev => ({
            ...prev,
          ...
          <div className="border-t pt-6">
            <label className="block text-sm font-medium text-gray-700 mb-3">
              <lucide_react_1.Share2 className="w-4 h-4 inline mr-1"/>
              Share Session
            </label>
            <div className="space-y-3">
              {!shareUrl ? (<button onClick={generateShareUrl} className="w-full px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors flex items-center justify-center gap-2">
                  <lucide_react_1.Link className="w-4 h-4"/>
                  Generate Share Link
                </button>) : (<div className="flex gap-2">
                  <input value={shareUrl} readOnly className="flex-1 px-3 py-2 border border-gray-300 rounded bg-gray-50 text-sm"/>
                  <button onClick={copyShareUrl} className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors flex items-center gap-2">
                    {copied ? <lucide_react_1.CheckCircle className="w-4 h-4"/> : <lucide_react_1.Copy className="w-4 h-4"/>}
                    {copied ? 'Copied' : 'Copy'}
                  </button>
                </div>)}
            </div>
        ...
        <div className="flex gap-3 mt-6 pt-6 border-t border-gray-200">
          ...
          <button onClick={handleExport} disabled={data.length === 0} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:bg-gray-300 disabled:cursor-not-allowed flex items-center gap-2">
